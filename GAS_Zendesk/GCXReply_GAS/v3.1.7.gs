...
(function () {
  ...
  async function ensureCustomerContextPanelOpen_() {
    ...
    let btn = null;
    for (let tries = 0; tries < 8 && !btn; tries++) { // ~8 * 250ms = 2s max wait
    ...
    if (buyerName) {
      ...
      ensureCustomerContextPanelOpen_().then(switchedTab => {
        ...
        const tryRead = () => {
          ...
          if (resolvedRequesterName || ++tries >= 8) { finish_(); return; } // ~8 * 250ms = 2s max wait
    ...
    function resolveNotesTextarea_(cb) {
      ensureCustomerContextPanelOpen_().then(switchedTab => {
        ...
        const findAll = () => {
          ...
          if (found.length || ++tries >= 8) { proceed_(found); return; } // ~8 * 250ms = 2s max wait
    ...
    function refreshNotes() {
      resolveNotesTextarea_(ta => {
        ...
        const poll = () => {
          ...
          if (ta.value || ++tries >= 8) return; // ~8 * 250ms = 2s max wait
          setTimeout(poll, 250);
        };
        poll();
      });
