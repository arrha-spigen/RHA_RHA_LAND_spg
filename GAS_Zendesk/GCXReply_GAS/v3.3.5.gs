...
(function () {
  ...
  async function relayAbmReply_(ticketId, htmlBody) {
    ...
    let resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    // Upload attachments once, before either send attempt, and reuse the same
    // uploaded refs on retry — re-uploading on a resourceVersion retry would
    ...
    const attachmentRefs = resourceVersion
      ...
      : [];
    let ok = resourceVersion && await sendScMessage_(base, match.caseId, match.marketplaceId, orderId, ticket.subject, match.csrfToken, resourceVersion, plainText, attachmentRefs);
    if (!ok) {
      _scResourceVersion = null;
      resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
      ok = resourceVersion && await sendScMessage_(base, match.caseId, match.marketplaceId, orderId, ticket.subject, match.csrfToken, resourceVersion, plainText, attachmentRefs);
    }
    logStep_(ok
