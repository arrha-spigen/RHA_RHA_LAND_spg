...
(function () {
  ...
  // v3.6.8 asked this on the Zendesk ticket page (via the "→ MCF" button) and
  // threaded the pick through the hash payload to the SC page — but the "→ MCF"
  // button is only ONE of the ways agents reach the SC MCF page; a Zendesk
  // Macro's hyperlink jumps straight there without ever running that code, so
  // the prompt never fired and the pick never got attached to that send.
  // v3.6.9 moves BOTH the prompt AND the storage to the SC MCF page itself
  // (inside autoFillFromHash_, right before markRowMcf_ — the actual moment a
  // row is about to be logged), using GM_setValue/GM_getValue instead of
  // saveUi/localStorage. GM storage is keyed per-SCRIPT, not per-origin, so
  // the same saved pick is visible from the Zendesk page's ⚙ settings AND
  // every sellercentral.amazon.* domain (DE/UK/JP/IN/...) without needing to
  // pass it through any payload at all.
  ...
  async function trySendPendingItem_(item) {
    ...
    const base = `https://sellercentral.${scDomain}`;
