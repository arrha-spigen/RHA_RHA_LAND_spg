...
(function () {
  ...
  // Asked once via showMcfPersonPicker_ (first MCF send after install/reset),
  // saved per-browser via saveUi({ mcfPerson }), changeable anytime in ⚙ settings.
  ...
  function buildMcfPayload_(panelEl, emailOverride, chosenAsin, person) {
  ...
  async function sendToMCF(panel) {
    ...
    // Ask once per browser (not on every send) — saved via saveUi, also
    // changeable anytime in ⚙ settings (see initSettings_). A cancel here
    // isn't fatal: the send still proceeds, just without 담당자 attribution
    // for this one row (MCF_Tracking's backend falls back to '김지우').
    let mcfPerson = loadUi().mcfPerson;
    if (!mcfPerson) {
      showSt('담당자 확인 중…');
      const picked = await showMcfPersonPicker_();
      if (picked) { mcfPerson = picked; saveUi({ mcfPerson: picked }); }
    }
    ...
    const payload = buildMcfPayload_(panel, ctxEmail, chosenAsin, mcfPerson);
  ...
  function initSettings_(panel) {
      ...
      <div class="sp-settings-inner">
        ...
        <div style="border-top:1px solid rgba(0,0,0,0.06);padding-top:8px;margin-top:8px;">
          ...
          <select id="sp-mcf-person-select" style="width:100%;padding:4px 6px;font-size:12px;border:1px solid #d8dcde;border-radius:6px;box-sizing:border-box;">
            ...
            ${MCF_PERSON_OPTIONS.map(p => `<option value="${p}" ${uiState.mcfPerson === p ? 'selected' : ''}>${p}</option>`).join('')}
          </select>
          <div style="font-size:10.5px;color:#999;margin-top:3px;line-height:1.4;">
    ...
    if (mcfPersonSelect) {
      mcfPersonSelect.addEventListener('change', () => {
        saveUi({ mcfPerson: mcfPersonSelect.value || null });
      });
    }
  ...
  function initMcfPage_() {
    ...
    // `person` comes from the payload built on the Zendesk side (see
    // showMcfPersonPicker_/sendToMCF) — falls back to the backend's own
    // '김지우' default (MCF_Tracking/tamperMonkey.js markMcfRow_) if the
    // agent hasn't set one yet, same as before this was per-user.
    ...
    async function autoFillFromHash_() {
      ...
      try {
        ...
        if (d.email) {
          const orderId = await markRowMcf_(d.email, d.person);
