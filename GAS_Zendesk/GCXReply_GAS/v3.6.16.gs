...
(function () {
  ...
  function initMcfPage_() {
    ...
    async function markRowMcf_(email, person) {
      if (!email) { console.log('[GCX MCF DIAG] markRowMcf_: no email, aborting'); return null; }
      const url = _MCF_GAS_EP + '?email=' + encodeURIComponent(email) + '&action=markMcf&match=last&person=' + encodeURIComponent(person || '');
      console.log('[GCX MCF DIAG] markRowMcf_ start', url);
      for (let attempt = 0; attempt < 3; attempt++) {
        const res = await gmGet_(url);
        console.log('[GCX MCF DIAG] attempt', attempt, 'status=', res && res.status, 'text=', res && res.responseText && res.responseText.slice(0, 400));
        if (res && res.status === 200) {
          try {
            const data = JSON.parse(res.responseText);
            console.log('[GCX MCF DIAG] parsed', data);
            return (data?.success === true && data.orderId) ? data.orderId : null;
          } catch (e) { console.log('[GCX MCF DIAG] JSON parse failed:', e.message); /* malformed response — fall through to retry */ }
        }
        if (attempt < 2) await sl(1000);
      }
      console.log('[GCX MCF DIAG] all 3 attempts exhausted, returning null');
      return null;
    }
    ...
    async function autoFillFromHash_() {
      ...
      try {
        ...
        console.log('[GCX MCF DIAG] reached email branch check, d.email=', d.email);
        if (d.email) {
      ...
      } catch(e) { console.error('[GCX MCF DIAG] autoFillFromHash_ threw:', e); }
    }
