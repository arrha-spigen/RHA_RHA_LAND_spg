...
(function () {
  ...
  async function trySendPendingItem_(item) {
    ...
    const fail = () => {
      logAbmRelayResult_(item.relayKey, item.ticketId, item.caseId, item.marketplace, 'failed', (Number(item.attempts) || 1) + 1, 'resend attempt failed', item.messageText, item.commentId);
      return false;
    };
    const base = `https://sellercentral.${item.marketplace}`;
    const fresh = await resolveScCaseDirect_(base, item.caseId).catch(() => null);
    if (!fresh) return fail(); // no usable SC session for that marketplace/case in this browser right now
    const resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    if (!resourceVersion) return fail();
    let attachmentRefs = [];
    if (item.commentId) {
    ...
    const ok = await sendScMessage_(base, item.caseId, fresh.marketplaceId, null, '', fresh.csrfToken, resourceVersion, item.messageText, attachmentRefs);
    if (!ok) return fail();
