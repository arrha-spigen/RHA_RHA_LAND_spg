...
(function () {
  ...
  const SCRIPT_VER = (typeof GM_info !== 'undefined' ? GM_info?.script?.version : null) || '2.11.5';
  ...
  const SP_GLASS_DEFAULTS_ = {
    blurRadius:    10.0,  // strong iOS-style frosted blur
    tintOpacity:   0.14,  // very subtle neutral frost (no gradient darkening)
    edgeIntensity: 0.038, // visible edge refraction
    rimIntensity:  0.062, // pronounced rim highlight
    baseIntensity: 0.018, // (only used when center warp enabled)
    edgeDistance:  0.090, // tight edge zone
    rimDistance:   0.360, // mid-range rim band
    baseDistance:  0.130, // (only used when center warp enabled)
    cornerBoost:   0.028, // corner sparkle
    rippleEffect:  0.140, // glassy ripple texture
    warp:          false, // center warp off — distortion stays at edges only
  };
  const SP_GLASS_LS_KEY_ = 'sp_glass_params_v3';
  ...
  class GlassWebGL_ {
    constructor(panel) {
      ...
      // Initial captures: 900ms (page settled) + 3500ms (lazy images loaded)
      setTimeout(() => { if (!this._dead) this._capture(); }, 900);
      setTimeout(() => { if (!this._dead) this._capture(); }, 3500);
      // Live background loop starts after initial captures finish
      setTimeout(() => { if (!this._dead) this._startLiveCapture(); }, 5000);
    }
    ...
    _buildProgram() {
      ...
      const fs = this._shader(gl.FRAGMENT_SHADER, `
        ...
        void main() {
          ...
          vec2 sNorm = normalize(coord - vec2(0.5, 0.5));
          ...
          // 7×7 Gaussian blur (~29 samples after radial skip — 4× cheaper than 13×13)
          vec4 color = vec4(0.0);
          vec2 ts = 1.0 / tSize;
          float sig = u_blurRadius / 2.0;
          float tw = 0.0;
    ...
    _startLiveCapture() {
      // Schedule next capture only when browser is idle → zero jank during user interaction.
      // requestIdleCallback fires between frames when the main thread has spare time.
      // timeout:8000 ensures it still fires within 8s even if the browser is always "busy".
      const tick = () => {
        if (this._dead) return;
        this._idleCb = requestIdleCallback(() => {
          this._idleCb = null;
          if (!this._dead && !this._capturing && document.visibilityState !== 'hidden') {
            this._capture();
          }
          // Next tick 4s after this one regardless of capture duration
          if (!this._dead) this._liveTimer = setTimeout(tick, 4000);
        }, { timeout: 8000 });
      };
      this._liveTimer = setTimeout(tick, 5000);
    }
    ...
    _capture() {
      ...
      // scale:0.3 → ~10× fewer pixels to rasterize; blur hides all quality loss.
      // _texW/_texH store VIEWPORT dims (not texture dims) so the shader UV math
      // (pagePixel / tSize) maps correctly to [0,1] regardless of capture scale.
      html2canvas(document.body, {
        ...
        // Store viewport dims — the texture represents the full viewport at any scale
        this._texW = vw;
        this._texH = vh;
        gl.bindTexture(gl.TEXTURE_2D, this._tex);
    ...
    destroy() {
      ...
      if (this._idleCb)  { cancelIdleCallback(this._idleCb); this._idleCb = null; }
    }
  }
  safeAddStyle_(`
  ...
  function makeDraggable(panel, handle) {
    handle.addEventListener('mousedown', e => {
      if (e.target.closest('#sp-settings-btn, #sp-minimize-btn, #sp-panel-close, .sp-re')) return;
      e.preventDefault();
      const rect = panel.getBoundingClientRect();
      ...
      const onMove = e2 => {
        ...
        // Re-render glass at new position using existing texture (no recapture)
        if (panel._glass) panel._glass._draw();
      };
      const onUp = () => {
    ...
    Object.entries(DIRS).forEach(([cls, h]) => {
      ...
      el.addEventListener('mousedown', e => {
        ...
        const onMove = ev => {
          ...
          // Sync canvas pixel size then re-render at new dimensions
          if (panel._glass) panel._glass._resize();
        };
