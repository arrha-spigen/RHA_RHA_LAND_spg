...
(function () {
  ...
  // ticket page — independent of panel/init state, since a reply can be
  // submitted before the floating panel ever finishes mounting.
  if (location.hostname.includes('zendesk.com') && /\/tickets\/\d+/.test(location.pathname)) {
  ...
  //
  // Request body shape for the send is inferred from the endpoint's own read model
  // (GET .../artifacts returns {message:{messageBody, preContext, postContext, ...}})
  // — not independently confirmed against a live send, since doing so would require
  // testing against a real customer case, which we do not do (see memory:
  // feedback_no_test_messages_to_real_abm_customers). If this shape is wrong, the
  // send fails cleanly (no artifact created) and logs a clear error — nothing is
  // ever delivered to a buyer on a failed/malformed attempt. The very next real
  // agent reply on a real ABM ticket is both the first real test and, if it works,
  // a genuine legitimate send — never a placeholder/probe message.
  ...
  function sendScMessage_(base, caseId, csrfToken, resourceVersion, text) {
    return new Promise(resolve => {
      GM_xmlhttpRequest({
        method: 'POST',
        ...
        data: JSON.stringify({ message: { messageBody: text } }),
        timeout: 15000,
        onload:    res => resolve(!!res && res.status >= 200 && res.status < 300),
        onerror:   () => resolve(false),
        ontimeout: () => resolve(false),
  ...
  async function relayAbmReply_(ticketId, htmlBody) {
    ...
    logStep_(`ABM relay: resolving SC case (order=${orderId || '—'}, asin=${asin || '—'})…`);
    let match = orderId ? await findScCaseForOrder_(base, orderId).catch(() => null) : null;
    ...
    let resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    let ok = resourceVersion && await sendScMessage_(base, match.caseId, match.csrfToken, resourceVersion, plainText);
    if (!ok) {
      _scResourceVersion = null;
      resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
      ok = resourceVersion && await sendScMessage_(base, match.caseId, match.csrfToken, resourceVersion, plainText);
    }
    logStep_(ok
      ? `ABM relay: sent to SC case ${match.caseId}`
      : `ABM relay: FAILED to send to SC case ${match.caseId} — reply is only in Zendesk, agent should also reply in Seller Central manually`);
    abmToast_(ok ? '✓ Also sent to the Amazon buyer via Seller Central' : '✗ Could NOT relay to Amazon — reply manually in Seller Central', ok);
  }
