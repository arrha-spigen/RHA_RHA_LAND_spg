...
const DEFS_PATCH = [
  ...
  {
    cat: FILM, label: '부착어려움', op: 'replace',
    text: "글라스를 부착하는 과정에서 정상적으로 부착하기 어렵거나 여러 번 실패했다고 언급하는 경우를 의미함. 예를 들어, \"붙이기 너무 어렵다\", \"계속 실패했다\", \"정렬이 힘들다\" 등의 표현이 해당됨. 이는 부착 후 발생한 문제(기포, 벗겨짐 등)가 아닌, 부착 과정 자체의 난이도에 대한 불만으로 분류함. (글라스를 붙이는 과정 자체가 어렵다고 느낀 경우) [중요] 리뷰의 핵심 불만이 '부착에 실패했다', '붙이지 못했다', '여러 장을 버렸다', '두 장 다 날렸다', '설치가 불가능했다'인 경우에는, 그 원인으로 가이드 프레임이나 컷아웃 크기 등이 함께 언급되어 있더라도 '프레임형합'이나 '컷아웃'이 아니라 반드시 본 항목으로 분류할 것. [중요] 단, 실패 또는 폐기의 원인이 먼지·이물질이 필름 사이에 들어갔기 때문이라고 명시된 경우(예: '이물질이 들어갔다', '먼지 때문에 버렸다')는 본 항목이 아니라 반드시 '먼지유입'으로 분류할 것 — 정렬/부착 난이도가 아닌 오염이 원인이기 때문임."
  },
  {
  ...
  /*======================================================
   * 5) 2026-08-06 2차 — 시트가 508행으로 늘어난 뒤 재검증에서
   *    발견된 신규 불일치 6건 (91.4%) 대응
   *====================================================*/

  // 5-1) 휴대폰케이스에 정의가 비어 있던 나머지 9개 항목 채우기
  //      (오배송이 이번 라운드 불일치 1건의 직접 원인으로 확인됨;
  //       나머지는 같은 실패 유형이라 예방적으로 함께 채움 —
  //       휴대폰보호필름 쪽 기존 정의를 케이스에 맞춰 이식)
  {
    cat: CASE, label: '구성품누락', op: 'replace',
    text: "케이스 제품의 구성품이 일부 누락된 채 배송되었다고 언급하는 경우를 의미함. 예를 들어 구성품 2개 중 1개만 배송된 경우, 제품 페이지 설명과 다르게 양면테이프·클리너·설명서 등 구성품이 포함되어 있지 않은 경우 등이 해당됨."
  },
  {
    cat: CASE, label: '동봉제품상이함', op: 'replace',
    text: "패키지에 동봉된 제품이나 구성품이 실제 주문한 것과 다르다고 언급하는 경우를 의미함."
  },
  {
    cat: CASE, label: '오배송', op: 'replace',
    text: "고객이 주문하지 않은 제품이 배송되었다고 언급하는 경우를 의미함. 예를 들어 주문한 케이스가 아닌 다른 제품(필름 등)이 배송된 경우, 주문한 색상이나 모델과 다른 제품이 배송된 경우 등이 해당됨. [중요] 배송 자체가 되지 않았다는 언급은 '미배송'으로 분류할 것."
  },
  {
    cat: CASE, label: '미배송', op: 'replace',
    text: "주문한 제품이 실제로 배송되지 않았다고 언급하는 경우를 의미함. 예를 들어 배송완료로 표시되었으나 실제로는 물건을 받지 못한 경우, 주문 후 오랜 시간이 지나도 발송조차 되지 않은 경우 등이 해당됨. [중요] 다른 제품이 대신 배송된 경우는 '오배송'으로 분류할 것."
  },
  {
    cat: CASE, label: '파손된상품수령', op: 'replace',
    text: "배송받은 케이스가 이미 파손된 상태로 도착했다고 언급하는 경우를 의미함."
  },
  {
    cat: CASE, label: '기기손상', op: 'replace',
    text: "정상적인 사용 범위 내에서 케이스로 인해 휴대폰 본체 외관에 스크래치, 도장 벗겨짐, 마모 등의 물리적 손상이 발생했다고 언급하는 경우를 의미함."
  },
  {
    cat: CASE, label: '법적대응언급', op: 'replace',
    text: "소비자보호원, 변호사, 고소 등 법적·제도적 대응 수단을 명시적으로 언급하는 경우를 의미함."
  },
  {
    cat: CASE, label: '신체상해주장', op: 'replace',
    text: "케이스 사용 과정에서 실제 신체 손상이나 피부 이상(알레르기, 발진 등)을 유발했다고 언급하는 안전(Safety) 관련 이슈를 의미함."
  },
  {
    cat: CASE, label: '워런티문의', op: 'replace',
    text: "제품의 보증기간 또는 보증 등록 절차에 관해 문의하는 경우를 의미함."
  },

  // 5-2) 51행: "지문+얼룩"이 본 리뷰의 핵심인데도 뒤에 곁들여진
  //      "필름이 들뜨네요"에 끌려가 자사/타사필름간섭으로 잘못 감 → 우선순위 명시
  {
    cat: CASE, label: '자사필름간섭', op: 'append',
    text: " [중요] 필름 관련 언급이 리뷰의 부수적인 디테일에 그치고, 지문·얼룩·자국 등 케이스 표면 자체의 문제가 리뷰의 핵심 불만인 경우에는 본 항목이 아니라 그 표면 문제에 해당하는 인입사유(자국/얼룩 등)로 분류할 것."
  },
  {
    cat: CASE, label: '타사필름간섭', op: 'append',
    text: " [중요] 필름 관련 언급이 리뷰의 부수적인 디테일에 그치고, 지문·얼룩·자국 등 케이스 표면 자체의 문제가 리뷰의 핵심 불만인 경우에는 본 항목이 아니라 그 표면 문제에 해당하는 인입사유(자국/얼룩 등)로 분류할 것."
  },

  // 5-3) 54행: "긁힘"이라는 명시적 단어가 있는데도 코팅벗겨짐으로 감 → 명시적 단어 우선
  {
    cat: CASE, label: '코팅벗겨짐', op: 'append',
    text: " [중요] 단순히 '긁힘', '스크래치', 'scratch' 등의 표현만 있고 코팅이나 도장층이 벗겨져 하부 소재가 노출된다는 명시적 언급이 없는 경우는 본 항목이 아니라 반드시 '스크래치'로 분류할 것."
  },

  // 5-4) 64행: MagSafe 언급이 전혀 없는데도(케이스 자체를 폰에 씌우는 어려움)
  //      "부착"이라는 단어에 끌려 부착어려움(MagSafe)로 감 → 기존 가드 강화
  {
    cat: CASE, label: '부착어려움(MagSafe)', op: 'append',
    text: " [중요] 케이스를 폰에 씌우거나 처음 장착하는 과정 자체가 어렵다는 언급(보호 스티커 제거, 설명서 부재, 초기 조립 시간이 오래 걸림 등)은 본 항목이 아님. MagSafe/MagFit 액세서리(충전기, 카드지갑, 거치대 등)를 케이스에 부착하는 과정에서의 어려움을 언급한 경우에만 선택할 것."
  }
];
