...
**Target sheets:** `iPhone 18` and `iPhone Fold` — both share the same layout: ASIN in col **B**, rating written to col **E** ("Rating") starting at row **6**
**Apify task:** `CLQ3G6Sokyr7AJtQi` (`product-details-scraper-iphone18-fold`) — one task, pre-loaded with amazon.de URLs for every ASIN across both sheets (185 URLs / 181 unique ASINs as of setup)
...
2. The poller checks run status; once `SUCCEEDED`, it fetches the run's dataset (`asin`, `productRating`), builds one ASIN → rating map, and applies it to **both** `iPhone 18!E6:E<lastRow>` and `iPhone Fold!E6:E<lastRow>`, each matched against its own `B` column (ASIN), writing `=HYPERLINK("https://www.amazon.de/dp/<ASIN>", <rating>)`. A row whose ASIN got no (or empty) result in that run is left completely untouched — an existing rating is only ever overwritten by a fresh non-empty scrape result, never blanked.
...
5. `reprocessDataset(datasetId)` re-applies an already-completed run's dataset to both sheets without starting a new Apify run — useful after a write-logic fix.
...
3. Optionally run **Apify Rating → Run Now (refresh ratings)** for an immediate refresh.
