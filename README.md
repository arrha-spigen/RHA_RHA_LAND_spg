...
| Project | Description | README |
|---------|-------------|--------|
| [Browser_Extensions/tampermonkey_scripts](Browser_Extensions/tampermonkey_scripts/) | **GCX Reply** (`v3.5.2`) — Zendesk order/product lookup panel, Auto-Fill, MCF handoff, ABM auto-relay + NRN. **Amazon MCF Autofill** (`v1.4.3`) — EU Seller Central MCF order-page autofill. **Amazon JP MCF Autofill** (`v1.5.2`) — JP variant. **Amazon Invoice Automation** (`v1.5`) — Amazon.de invoice download. **GChat Reply Suggest** (`v2.1.1`) — Alt+G suggests 3 AI reply sentences in most Google Chat rooms (incl. the Chrome-PWA desktop app), backed by a local server (`Browser_Extensions/gchat_reply_suggest_server.py`) that calls the `claude` CLI directly. In designated "ticket forward" rooms (matched by space ID, with ↑/↓ ticket browsing) it instead offers a deterministic, no-AI ticket-forward template — bold + real hyperlink — sourced from recently-visited Zendesk tickets. Install `.user.js` files via Tampermonkey Dashboard → Import. | [README](Browser_Extensions/tampermonkey_scripts/README.md) |
