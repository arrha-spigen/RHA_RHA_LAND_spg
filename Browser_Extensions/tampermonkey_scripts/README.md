...
- Alt+G shows that list (most recent pre-highlighted); **↑/↓ arrow keys browse older tickets, Enter confirms** (mouse click also works). Picking one opens a second picker — **who to @mention** (defaults to `(no mention)`, plus every sender seen in the visible conversation) — then a third picker — **which honorific** (`리더`/`파트장`/`프로`, defaults to `프로` — configured in `HONORIFIC_TITLES`/`DEFAULT_HONORIFIC` near the top of the script) — then inserts:
  ```
