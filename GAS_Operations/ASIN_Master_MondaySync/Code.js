...
/*************************************************
 * ABM_RELAY_LOG CLEANUP
 * ------------------------------------------------
 * Fully independent of the Monday.com sync above — everything here operates
 * on a DIFFERENT tab ("ABM_Relay_Log") in this same spreadsheet, written by
 * a separate project (GCXReply_GAS's upsertAbmRelayLog_ — see
 * ~/Desktop/GCX/GCXReply_GAS/Code.js). Nothing here reads or modifies
 * BOARD_ID, syncMondayBoardToSheet, or any other symbol above; it doesn't
 * even touch onOpen()'s menu.
 *
 * Why: that log only ever grows (one row per unique ticket+comment ABM
 * relay, upserted in place, never pruned), and upsertAbmRelayLog_ does a
 * full sheet.getDataRange().getValues() scan on EVERY single write just to
 * find a matching RelayKey to update — so letting the sheet grow without
 * bound makes every ABM reply relay across every agent's browser
 * progressively slower, not just a cosmetic sheet-size issue. This prunes
 * rows whose Timestamp (col A, written as a real Date by upsertAbmRelayLog_)
 * is older than ABM_LOG_RETENTION_DAYS_. Safe to run anytime: a row only
 * ever stays 'pending'/'sending' for at most a few minutes (see
 * ABM_PENDING_STALE_MS/ABM_SENDING_STALE_MS in GCXReply_GAS) before settling
 * to success/failed, so nothing genuinely in-flight can ever be 15 days old.
 *************************************************/
const ABM_LOG_SHEET_NAME_ = 'ABM_Relay_Log';
const ABM_LOG_RETENTION_DAYS_ = 15;

function cleanupOldAbmRelayLogRows() {
  const sheet = SpreadsheetApp.getActive().getSheetByName(ABM_LOG_SHEET_NAME_);
  if (!sheet) { Logger.log('cleanupOldAbmRelayLogRows: sheet "' + ABM_LOG_SHEET_NAME_ + '" not found — nothing to do.'); return; }

  const lastRow = sheet.getLastRow();
  if (lastRow < 2) { Logger.log('cleanupOldAbmRelayLogRows: no data rows.'); return; }

  const cutoff = new Date(Date.now() - ABM_LOG_RETENTION_DAYS_ * 24 * 60 * 60 * 1000);
  const timestamps = sheet.getRange(2, 1, lastRow - 1, 1).getValues(); // col A = Timestamp

  const oldRows = [];
  for (let i = 0; i < timestamps.length; i++) {
    const v = timestamps[i][0];
    const d = (v instanceof Date) ? v : (v ? new Date(v) : null);
    if (d && !isNaN(d) && d < cutoff) oldRows.push(i + 2); // +2: 1-indexed + header row
  }
  if (!oldRows.length) { Logger.log('cleanupOldAbmRelayLogRows: nothing older than ' + ABM_LOG_RETENTION_DAYS_ + ' days (' + (lastRow - 1) + ' rows checked).'); return; }

  // Delete contiguous runs highest-row-first, so a deletion never shifts the
  // row number of a still-queued (lower) row waiting to be deleted.
  let end = oldRows[oldRows.length - 1];
  let start = end;
  for (let i = oldRows.length - 2; i >= 0; i--) {
    if (oldRows[i] === start - 1) { start = oldRows[i]; continue; }
  ...
  Logger.log('cleanupOldAbmRelayLogRows: deleted ' + oldRows.length + ' row(s) older than ' + ABM_LOG_RETENTION_DAYS_ + ' days.');
}

// One-time setup — run manually from the Apps Script editor (Run dropdown),
// NOT via the API: time-trigger creation needs the script.scriptapp OAuth
// scope, which 403s headlessly until a human clicks through the consent
// dialog once (same gotcha documented for ABM_TicketMerge's
// setupReconcileTrigger). Installs a daily trigger; safe to re-run (no-ops
// if already installed).
function installAbmRelayLogCleanupTrigger() {
  const exists = ScriptApp.getProjectTriggers().some(t => t.getHandlerFunction() === 'cleanupOldAbmRelayLogRows');
  if (exists) { Logger.log('cleanupOldAbmRelayLogRows trigger already exists.'); return; }
  ScriptApp.newTrigger('cleanupOldAbmRelayLogRows').timeBased().everyDays(1).atHour(4).create();
  Logger.log('Installed daily trigger for cleanupOldAbmRelayLogRows (runs ~4am Asia/Seoul).');
}
