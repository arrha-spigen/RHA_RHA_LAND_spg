...
(function () {
  ...
  async function ensureCustomerContextPanelOpen_() {
    ...
    let btn = null;
    for (let tries = 0; tries < 40 && !btn; tries++) { // ~40 * 250ms = 10s max wait
      btn = document.querySelector('[data-test-id="omnipanel-selector-item-customer-context"]');
    ...
    if (buyerName) {
      ...
      ensureCustomerContextPanelOpen_().then(switchedTab => {
        ...
        const tryRead = () => {
          ...
          if (resolvedRequesterName || ++tries >= 20) { finish_(); return; } // ~20 * 250ms = 5s max wait
  ...
  function init() {
    ...
    function resolveNotesTextarea_(cb) {
      ensureCustomerContextPanelOpen_().then(switchedTab => {
        ...
        const findAll = () => {
          ...
          if (found.length || ++tries >= 20) { proceed_(found); return; } // ~20 * 250ms = 5s max wait
          setTimeout(findAll, 250);
        };
        findAll();
    ...
    function refreshNotes() {
      resolveNotesTextarea_(ta => {
        if (!ta) { notesContent.value = ''; return; }
        ...
        const poll = () => {
          ...
          if (ta.value || ++tries >= 20) return; // ~20 * 250ms = 5s max wait
          setTimeout(poll, 250);
        };
        poll();
      });
