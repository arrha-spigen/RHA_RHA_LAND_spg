    ...
    return `
      ...
      <div class="sp-block" data-sp-section="order">
        ...
        <div class="sp-block-body">
          ...
          <div id="sp-seller-notes-wrap">${
            data.sellerNotes !== undefined
              ? row('Seller Notes', data.sellerNotes)
              : `<div class="sp-row"><span class="sp-label">Seller Notes</span><span class="sp-val">${SPINNER_HTML}</span></div>`
          }</div>
        ...
        try {
          ...
          fetchScBuyerStats_(orderId, data.order?.SalesChannel, data.address?.CountryCode, stats => {
            ...
            if (stats.sellerNotes != null) {
              const notesWrap = result.querySelector('#sp-seller-notes-wrap');
              if (notesWrap) notesWrap.innerHTML = row('Seller Notes', stats.sellerNotes);
            }
            maybeShowAutoFill(document.getElementById(PANEL_ID));
          });
