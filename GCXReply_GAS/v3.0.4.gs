...
(function () {
  ...
  function showFillConfirm_(rows, onConfirm, onCancel) {
        ...
        <tbody id="sp-confirm-tbody">
          ${rows.map((r, i) => {
            ...
            if (hasOpts_(r) || isMulti) {
              ...
              // Native date input: calendar picker on click (via .showPicker(),
              // wired up below) + free yyyy-mm-dd typing. lang="sv-SE" forces
              // ISO (yyyy-mm-dd) display/typing order regardless of OS locale —
              // same trick purchaseDateLocal_() uses for the ISO string itself.
              // .value on a date input is always yyyy-mm-dd, so this slots
              // straight into apiVal with no reformatting needed.
              afterTd = `<td style="padding:3px 8px;">
                   <input type="date" lang="sv-SE" data-textinput="${i}" value="${esc_(r.after)}"
    ...
    // Date input (Purchase Date): open the native calendar picker on any
    // click in the box, not just the small calendar-icon glyph — matches how
    // every other "click the field" combobox/text input in this modal behaves.
    card.querySelector('#sp-confirm-tbody').addEventListener('click', e => {
      if (e.target.matches('input[type="date"]')) {
        try { e.target.showPicker(); } catch (_) {} // unsupported browsers: icon click still works natively
      }
    });
  ...
  function fetchScBuyerStats_(orderId, salesChannel, countryCode, cb) {
    ...
    function countByEmail_(email) {
      ...
      GM_xmlhttpRequest({
        ...
        onload(res) {
          ...
          if (!orderIds.length) { cb({ email, totalPurchases: count, totalRefunds: null, totalItemsPurchased: null, totalItemsRefunded: null }); return; }
          ...
          cb({ email, totalPurchases: count, totalRefunds: -1, totalItemsPurchased: null, totalItemsRefunded: null });
    ...
    function checkRefunds_(email, count, orderIds) {
      ...
      const finish_ = () => cb({
        ...
        totalItemsPurchased: itemQty, totalItemsRefunded: refundItemQty,
      });
