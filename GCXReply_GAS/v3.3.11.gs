...
(function () {
  ...
  function abmCaseIdFromAddress_(address) {
    const m = (address || '').match(/\+([0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12})@marketplace\./i);
    return m ? m[1] : null;
  }
  ...
  // Uploads every attachment on the just-created Zendesk comment to the matched
  // SC case, skipping (with a log line, never a hard failure) any that fail
  // individually or that would push the total over SC's 10 MB composer limit —
  // the message text still sends even if an attachment can't be relayed.
  async function relayAbmAttachments_(base, caseId, marketplaceId, csrfToken, resourceVersion, ticketId) {
    ...
    if (!zdAttachments.length) return [];
    let totalBytes = 0;
    const uploaded = [];
  ...
  // setTimeout(800ms) fires). This is deliberately NOT gated behind the Debug log
  // setting: whether a reply actually reached the buyer is too consequential to be
  // silent by default.
  function abmToast_(msg, ok) {
  ...
  async function relayAbmReply_(ticketId, htmlBody) {
    ...
    const directCaseId = abmCaseIdFromAddress_(fromAddress);
    ...
    const attachmentRefs = resourceVersion
      ...
      : [];
    const { ok, attempts } = await sendAbmMessageWithRetry_(base, match, orderId, ticket.subject, plainText, attachmentRefs);
    logStep_(ok
      ? `ABM relay: sent to SC case ${match.caseId} (attempt ${attempts})`
      : `ABM relay: FAILED to send to SC case ${match.caseId} after ${attempts} attempts — will keep retrying automatically; reply is only in Zendesk for now, agent should also reply in Seller Central manually if urgent`);
    abmToast_(ok
    ...
    logAbmRelayResult_(relayKey, ticketId, match.caseId, domain, ok ? 'success' : 'failed', attempts, ok ? '' : 'exhausted retries', plainText);
  ...
  async function sendAbmMessageWithRetry_(base, match, orderId, subject, plainText, attachmentRefs) {
    ...
    for (let attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
      ...
      const ok = resourceVersion && await sendScMessage_(base, match.caseId, match.marketplaceId, orderId, subject, csrfToken, resourceVersion, plainText, attachmentRefs);
  ...
  function logAbmRelayResult_(relayKey, ticketId, caseId, marketplace, status, attempts, lastError, messageText) {
    GM_xmlhttpRequest({
      method: 'POST',
      url: GAS_URL,
      headers: { 'Content-Type': 'application/json' },
      data: JSON.stringify({ action: 'logAbmRelay', relayKey, ticketId, caseId, marketplace, status, attempts, lastError, messageText }),
      timeout: 15000,
  ...
  async function updateAbmBadge_() {
    const res = await gmGet_(`${GAS_URL}?action=abmRelayPending`);
    let pending = [];
    if (res && res.status === 200) {
      try { pending = JSON.parse(res.responseText).pending || []; } catch (_) {}
    }
    const sig = _abmPendingSig_(pending);
    if (!pending.length || sig === _abmDismissedSig_) {
    ...
    _abmBadgeEl_.querySelector('#sp-abm-badge-txt').textContent =
      `⚠ ${pending.length} ABM ${pending.length === 1 ? 'reply' : 'replies'} not yet delivered`;
  }
  ...
  async function renderAbmPanelBody_(panel) {
    ...
    const res = await gmGet_(`${GAS_URL}?action=abmRelayAll&limit=40`);
    let rows = [];
    if (res && res.status === 200) {
      try { rows = JSON.parse(res.responseText).rows || []; } catch (_) {}
    }
    // Guard against junk/migration rows: a real row always has a numeric ticket id.
    rows = rows.filter(r => /^\d+$/.test(String(r.ticketId)));
    const undelivered = rows.filter(r => r.status !== 'success');
    const delivered   = rows.filter(r => r.status === 'success');
    ...
    body.innerHTML = `
      <div style="font-weight:700;color:#c0392b;margin-bottom:4px;">Undelivered (${undelivered.length})</div>
    ...
    body.querySelectorAll('[data-abm-retry]').forEach(btn => btn.onclick = async () => {
      ...
      abmToast_(ok ? `✓ Ticket #${item.ticketId} reply delivered` : `✗ Ticket #${item.ticketId} retry failed — check the Seller Central login for ${item.marketplace}`, ok);
      await renderAbmPanelBody_(panel);
      updateAbmBadge_();
    });
  ...
  // Single-item resend, shared by the background sweep and the panel's
  ...
  async function trySendPendingItem_(item) {
    ...
    if (!fresh) return false; // no usable SC session for that marketplace/case in this browser right now
    const resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    if (!resourceVersion) return false;
    const ok = await sendScMessage_(base, item.caseId, fresh.marketplaceId, null, '', fresh.csrfToken, resourceVersion, item.messageText, []);
    if (ok) logAbmRelayResult_(item.relayKey, item.ticketId, item.caseId, item.marketplace, 'success', (Number(item.attempts) || 1) + 1, '', item.messageText);
    return ok;
  }
