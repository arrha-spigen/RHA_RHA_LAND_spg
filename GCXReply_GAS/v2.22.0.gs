...
(function () {
  ...
  function showFillConfirm_(rows, onConfirm, onCancel) {
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11.5px;">
        ...
        <tbody id="sp-confirm-tbody">
          ${rows.map((r, i) => {
            ...
            const afterTd = isMulti
              ? `<td style="padding:3px 8px;">
                   <select data-candidate="${i}" style="font-size:11px;width:100%;max-width:190px;border:1px solid #b2d4be;border-radius:4px;padding:2px 5px;color:#1a6e3a;font-weight:600;background:#f4fbf7;">
                     ${r.candidates.map(c => `<option value="${esc_(c.val)}"${c.val === r.api?.value ? ' selected' : ''}>${esc_(c.displayName)}</option>`).join('')}
                   </select>
                 </td>`
            ...
            return `<tr data-idx="${i}" style="border-bottom:1px solid #f2f4f7;${rowStyle}">
              <td style="padding:5px 6px;text-align:center;">
                <input type="checkbox" data-row="${i}" style="margin:0;cursor:pointer;"
                  ${hasValue && !noChange ? 'checked' : ''} ${!hasValue ? 'disabled' : ''}>
              </td>
    ...
    card.querySelector('#sp-confirm-ok').addEventListener('click', () => {
      ...
      // Resolve candidate <select> choices — update api.value and after before confirming
      card.querySelectorAll('select[data-candidate]').forEach(selEl => {
        const idx = +selEl.dataset.candidate;
        const row = rows[idx];
        if (!row?.candidates) return;
        const chosen = row.candidates.find(c => c.val === selEl.value);
        if (chosen) {
          if (row.api) row.api.value = chosen.val;
          row.after = chosen.displayName;
        }
      });
      overlay.remove();
  ...
  // ── No Response Needed — direct ChannelReply API ──────────────────────────
  // Previously bridged via postMessage into ChannelReply's cross-origin app
  // iframe (clicking their real button). Replaced with direct calls to the
  // same endpoints ChannelReply's own Angular app uses internally (reverse-
  // engineered from their public controllers.js + a live network capture),
  // so this no longer depends on ChannelReply's UI/iframe being mounted at
  // all — needed since Spigen is unsubscribing from the ChannelReply Zendesk app.
  //
  // Flow: GET /app/ticket-data/{ticketId}/1 → data.noResponseNeededId (absent
  // if not an Amazon Buyer Message ticket, or already marked) → GET
  // /amazon/update-no-response-needed/{id} to mark it. The id is short-lived
  // (observed to change between fetches), so it must be fetched fresh right
  // before each use, never cached.
  function fetchCrNrnId_(ticketId, cb) {
    GM_xmlhttpRequest({
      method:  'GET',
      url:     `https://www.channelreply.com/app/ticket-data/${ticketId}/1?forceRequest=1`,
      headers: { 'Accept': 'application/json, text/plain, */*', token: CR_APP_TOKEN, crm: '1' },
      timeout: 15000,
      onload(res) {
        if (res.status !== 200) { logStep_(`NRN id fetch: HTTP ${res.status}`); cb(null); return; }
        try {
          const d = JSON.parse(res.responseText);
          cb((d && d.result && d.data && d.data.noResponseNeededId) || null);
        } catch (e) { logStep_(`NRN id fetch: parse error — ${e.message}`); cb(null); }
      },
      onerror()   { logStep_('NRN id fetch: network error'); cb(null); },
      ontimeout() { logStep_('NRN id fetch: timeout'); cb(null); },
    });
  }

  function updateCrNrn_(id, cb) {
    GM_xmlhttpRequest({
      method:  'GET',
      url:     `https://www.channelreply.com/amazon/update-no-response-needed/${id}`,
      headers: { 'Accept': 'application/json, text/plain, */*', token: CR_APP_TOKEN, crm: '1' },
      timeout: 15000,
      onload(res) {
        if (res.status !== 200) { logStep_(`NRN update: HTTP ${res.status}`); cb(false); return; }
        try { cb(JSON.parse(res.responseText).status === 'success'); }
        catch (e) { logStep_(`NRN update: parse error — ${e.message}`); cb(false); }
      },
      onerror()   { logStep_('NRN update: network error'); cb(false); },
      ontimeout() { logStep_('NRN update: timeout'); cb(false); },
    });
  }

  // Re-checks button enabled/disabled state from the API. Exposed on window so
  // markNoResponseNeeded() can trigger a refresh after marking without duplicating logic.
  function refreshNrnState_() {
    ...
    if (!ticketId) { setBtn(true, 'Only available on Amazon Buyer Message tickets'); return; }
    fetchCrNrnId_(ticketId, id => {
      if (location.pathname.match(/\/tickets\/(\d+)/)?.[1] !== ticketId) return; // navigated away mid-flight
      setBtn(!id, id ? 'Mark as No Response Needed' : 'Only available on Amazon Buyer Message tickets (or already marked)');
    });
  }
  ...
  function markNoResponseNeeded(panel) {
    ...
    const ticketId = location.pathname.match(/\/tickets\/(\d+)/)?.[1];
    if (!ticketId) { show('✗ No ticket ID found', '#c0392b'); return; }
    ...
    fetchCrNrnId_(ticketId, id => {
      if (!id) {
        show('Already marked / not an Amazon Buyer Message ticket', '#888');
        logStep_('NRN: no pending message id (already marked or not ABM)');
        refreshNrnState_();
        return;
      }
      updateCrNrn_(id, ok => {
        if (ok) {
          ...
          logStep_(`NRN: marked via direct ChannelReply API (id=${id})`);
        } else {
          show('✗ Failed to mark', '#c0392b');
          logStep_(`NRN: direct API update failed (id=${id})`);
        }
        refreshNrnState_();
      });
    });
    ...
    function buildAndShow() {
      ...
      const apiOnlyRows = [
        { label: 'Device*', before: resolvedDevice?.opts?.find(o => o.value === currentCfMap[ZD.DEVICE])?.name || currentCfMap[ZD.DEVICE] || '',
          ...
          candidates: resolvedDevice?.candidates || null },
        { label: 'Product Name *', before: '', // resolved below using product opts
          ...
          candidates: resolvedProduct?.candidates || null },
      ...
      showFillConfirm_(allRows, selectedRows => {
        ...
        clearAllZdFields_(); // wipe stale React state at fill-time, not at navigation-time
  ...
  function init() {
    ...
    // NRN button state — direct ChannelReply API check (see refreshNrnState_
    // above), replacing the old postMessage bridge into ChannelReply's iframe.
    // Only re-checks on ticket change, not continuously.
