      ...
      GM_xmlhttpRequest({
        ...
        onload(res) {
          ...
          if (res.status === 200) {
            try {
              const d = JSON.parse(res.responseText);
              // SC search response uses "total", not "totalCount"/"totalOrders"
          ...
          // RefundApplied is on the individual order endpoint, not search results.
          // Fire all per-order requests in parallel and count when all done.
          let refundCount = 0;
            ...
            GM_xmlhttpRequest({
              ...
              onload(r2) {
                try {
                  ...
                  if (status?.RefundApplied === true) refundCount++;
                } catch {}
                done_();
              },
              onerror()   { done_(); },
