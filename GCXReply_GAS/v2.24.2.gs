...
(function () {
  ...
  const CHANGELOG_ = {
    ...
    // '2.24.2' entry pending — description to be confirmed with user.
    '2.24.1': [
  ...
  const SPINNER_HTML = `<span class="sp-spinner" role="img" aria-label="로딩중">${'<i></i>'.repeat(12)}</span>`;
  ...
  function showFillConfirm_(rows, onConfirm, onCancel) {
      ...
      <table style="width:100%;border-collapse:collapse;font-size:11.5px;">
        ...
        <tbody id="sp-confirm-tbody">
          ${rows.map((r, i) => {
            ...
            const afterTd = hasOpts_(r) || isMulti
              ? `<td style="padding:3px 8px;">
                   <select data-select="${i}" style="font-size:11px;width:100%;max-width:190px;border:1px solid #b2d4be;border-radius:4px;padding:2px 5px;color:#1a6e3a;font-weight:600;background:#f4fbf7;">
                     ${!r.api?.value ? '<option value="">—</option>' : ''}
                     ${selectOpts.map(o => `<option value="${esc_(o.value)}"${o.value === r.api?.value ? ' selected' : ''}>${esc_(o.name)}</option>`).join('')}
                   </select>
                 </td>`
              : `<td style="padding:3px 8px;">
    ...
    card.querySelector('#sp-confirm-tbody').addEventListener('change', e => {
      ...
      // Editing the After select/text-input implies the agent wants it included.
      if (e.target.matches('select[data-select], input[data-textinput]')) {
        const idx = +(e.target.dataset.select ?? e.target.dataset.textinput);
    ...
    card.querySelector('#sp-confirm-ok').addEventListener('click', () => {
      ...
      // what makes the After column actually editable (dropdown for tagger
      // fields, text input for free-text fields), not just a static preview.
      card.querySelectorAll('select[data-select]').forEach(selEl => {
        const idx = +selEl.dataset.select;
        const row = rows[idx];
        if (!row) return;
        const opt = selEl.selectedOptions[0];
        const val = opt ? opt.value : '';
        if (row.api) row.api.value = val;
        row.after = val ? opt.textContent : '';
      });
  ...
  function parseTicketAddress_(txt) {
    ...
    for (const line of t.split('\n').map(s => s.trim()).filter(Boolean)) {
      ...
      // Only build a block once a real "1) Full Name" line has opened one — a
      // ticket body can contain other, unrelated numbered lists (e.g. internal
      // processing notes "2. ... 3. ... 4. ... 5. ...") that must NOT be
      // mistaken for a continuation of the address block just because their
      // numbering happens to overlap.
      if (!cur) continue;
  ...
  function autoFillTicket(panel) {
    ...
    function buildAndShow() {
      ...
      const apiOnlyRows = [
        ...
        { label: 'Country*',      before: currentCfMap[ZD.COUNTRY]     || '', after: COUNTRY_MAP[ad.CountryCode] || '', api: COUNTRY_MAP[ad.CountryCode] ? { id: ZD.COUNTRY, value: COUNTRY_MAP[ad.CountryCode] } : null, opts: COUNTRY_OPTS },
        { label: 'Point of Purchase', before: currentCfMap[ZD.POINT_OF_PUR] || '', after: pop || '', api: pop ? { id: ZD.POINT_OF_PUR, value: pop } : null, opts: POP_OPTS },
        { label: 'Amazon Fulfillment Methods*', before: fulfillOpts.find(o => o.value === currentCfMap[ZD.FULFILLMENT])?.name || currentCfMap[ZD.FULFILLMENT] || '', after: resolvedFulfill?.name || '', api: resolvedFulfill?.val ? { id: ZD.FULFILLMENT, value: resolvedFulfill.val } : null, opts: fulfillOpts },
        { label: 'Brand(상세)*', before: BRAND_TAG_LABEL[currentCfMap[ZD.BRAND_DETAIL]] || currentCfMap[ZD.BRAND_DETAIL] || '', after: brandTag || '', api: brandTag ? { id: ZD.BRAND_DETAIL, value: brandTag } : null, opts: BRAND_OPTS },
        { label: '✅전체 주문 (Product Issue, 아크테크X)*', before: currentCfMap[ZD.TOTAL_ORDERS]  || '', after: purchasesVal || '', api: purchasesVal ? { id: ZD.TOTAL_ORDERS, value: purchasesVal } : null, opts: Q_OPTS },
        { label: '❎전체 환불*', before: currentCfMap[ZD.TOTAL_REFUNDS] || '', after: refundsVal || '', api: refundsVal ? { id: ZD.TOTAL_REFUNDS, value: refundsVal } : null, opts: Q_OPTS },
        { label: '❗사진/영상 유무❗*', before: currentCfMap[ZD.PHOTO_EXIST] || '', after: resolvedHasPhoto != null ? (resolvedHasPhoto ? 'yes' : 'no') : '', api: resolvedHasPhoto != null ? { id: ZD.PHOTO_EXIST, value: resolvedHasPhoto ? 'yes' : 'no' } : null, opts: PHOTO_OPTS },
      ];
  ...
  safeAddStyle_(`
    ...
    @keyframes sp-ios-spin { from { opacity: 1; } to { opacity: 0.15; } }
    .sp-spinner {
    ...
    .sp-spinner i {
      ...
      animation: sp-ios-spin 1.2s linear infinite;
      opacity: 0.15;
      font-style: normal;
    }
    .sp-spinner i:nth-child(1)  { transform: rotate(  0deg); animation-delay: -1.10s; }
    .sp-spinner i:nth-child(2)  { transform: rotate( 30deg); animation-delay: -1.00s; }
    .sp-spinner i:nth-child(3)  { transform: rotate( 60deg); animation-delay: -0.90s; }
    .sp-spinner i:nth-child(4)  { transform: rotate( 90deg); animation-delay: -0.80s; }
    .sp-spinner i:nth-child(5)  { transform: rotate(120deg); animation-delay: -0.70s; }
    .sp-spinner i:nth-child(6)  { transform: rotate(150deg); animation-delay: -0.60s; }
    .sp-spinner i:nth-child(7)  { transform: rotate(180deg); animation-delay: -0.50s; }
    .sp-spinner i:nth-child(8)  { transform: rotate(210deg); animation-delay: -0.40s; }
    .sp-spinner i:nth-child(9)  { transform: rotate(240deg); animation-delay: -0.30s; }
    .sp-spinner i:nth-child(10) { transform: rotate(270deg); animation-delay: -0.20s; }
    .sp-spinner i:nth-child(11) { transform: rotate(300deg); animation-delay: -0.10s; }
    .sp-spinner i:nth-child(12) { transform: rotate(330deg); animation-delay:  0.00s; }
  ...
  function fetchScBuyerStats_(orderId, salesChannel, countryCode, cb) {
    ...
    function countByEmail_(email) {
      ...
      GM_xmlhttpRequest({
        ...
        onload(res) {
          ...
          if (!orderIds.length) { cb({ email, totalPurchases: count, totalRefunds: null }); return; }
          ...
          cb({ email, totalPurchases: count, totalRefunds: -1 });
    ...
    // Per-order refund check: uses _scRefundCache_ to skip already-known orders.
    // All uncached orders are fetched in parallel via GM_xmlhttpRequest.
    function checkRefunds_(email, count, orderIds) {
      let refundCount = 0;
      const uncached = [];
      orderIds.forEach(oid => {
        if (_scRefundCache_[oid] !== undefined) {
          if (_scRefundCache_[oid]) refundCount++;
        } else {
          uncached.push(oid);
        }
      });
      if (!uncached.length) { cb({ email, totalPurchases: count, totalRefunds: refundCount }); return; }

      let pending = uncached.length;
      function done_() { if (--pending === 0) cb({ email, totalPurchases: count, totalRefunds: refundCount }); }
      ...
      uncached.forEach(oid => {
        GM_xmlhttpRequest({
          ...
          onload(r2) {
            try {
              const status = (JSON.parse(r2.responseText)?.order || {}).orderStatus;
              const val = status?.RefundApplied === true;
      ...
      onload(res) {
        ...
        try {
          ...
          fetchScBuyerStats_(orderId, data.order?.SalesChannel, data.address?.CountryCode, stats => {
            ...
            if (stats.totalPurchases == null && stats.totalRefunds == null && !stats.email) return;
