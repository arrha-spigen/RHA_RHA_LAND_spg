    ...
    function countByEmail_(email) {
      ...
      GM_xmlhttpRequest({
        ...
        onload(res) {
          ...
          if (res.status === 200) {
            try {
              ...
              if (Array.isArray(d.orders) && d.orders.length > 0) {
                // Log first order keys once to help identify refund field name
                console.log('[GCX] SC order[0] keys:', Object.keys(d.orders[0]));
                console.log('[GCX] SC order[0]:', JSON.stringify(d.orders[0]));
                refundCount = d.orders.filter(o =>
                  (typeof o.refundCount     === 'number' && o.refundCount > 0) ||
                  (typeof o.refundAmount    === 'number' && o.refundAmount > 0) ||
                  (typeof o.totalRefundAmount === 'number' && o.totalRefundAmount > 0) ||
                  (typeof o.adjustmentCount === 'number' && o.adjustmentCount > 0) ||
                  (o.hasRefund === true) ||
          ...
          cb({ email, totalPurchases: count, totalRefunds: refundCount });
        },
