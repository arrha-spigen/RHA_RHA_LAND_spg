...
(function () {
  ...
  const SCRIPT_VER = (typeof GM_info !== 'undefined' ? GM_info?.script?.version : null) || '2.11.1';
  ...
  const SP_GLASS_LS_KEY_ = 'sp_glass_params_v1';
  ...
  class GlassWebGL_ {
    constructor(panel) {
      ...
      // Initial capture after page has settled
      setTimeout(() => { if (!this._dead) this._capture(); }, 900);
      // Second capture for lazy-loaded images
      setTimeout(() => { if (!this._dead) this._capture(); }, 3500);

      // Recapture when the visible content behind the panel changes
      // (Zendesk scrolls internally, not window — use capture:true to catch it)
      this._onScroll = () => this._schedCapture(450);
    ...
    _buildProgram() {
      ...
      const fs = this._shader(gl.FRAGMENT_SHADER, `
        ...
        void main() {
          ...
          vec3 tint = mix(vec3(1.0), vec3(0.7), coord.y);
          color.rgb = mix(color.rgb, tint, u_tintOpacity);

          vec3 tClr = vec3(0.0), mClr = vec3(0.0), bClr = vec3(0.0);
          float sc = 0.0;
          float ty = (cCenter.y - cSize.y * 0.4) / tSize.y;
          float my = cCenter.y / tSize.y;
          float by = (cCenter.y + cSize.y * 0.4) / tSize.y;
          for (float x = 0.0; x < 1.0; x += 0.05) {
            for (float dy = -5.0; dy <= 5.0; dy += 1.0) {
              tClr += texture2D(u_image, vec2(x, ty + dy * ts.y)).rgb;
              mClr += texture2D(u_image, vec2(x, my + dy * ts.y)).rgb;
              bClr += texture2D(u_image, vec2(x, by + dy * ts.y)).rgb;
              sc += 1.0;
            }
          }
          tClr /= sc; mClr /= sc; bClr /= sc;
          vec3 sampG;
          if (coord.y < 0.1) {
            sampG = tClr;
          } else if (coord.y > 0.9) {
            ...
            float t2 = (coord.y - 0.1) / 0.8;
            sampG = t2 < 0.5 ? mix(tClr, mClr, t2 * 2.0) : mix(mClr, bClr, (t2 - 0.5) * 2.0);
          }
          color.rgb = mix(color.rgb, sampG, u_tintOpacity * 0.3);
    ...
    _capture() {
      if (typeof html2canvas !== 'function' || !this._prog || this._dead) return;
      const panel = this.panel;
      // Capture only the VIEWPORT (not full body scroll height).
      // Zendesk uses an internal scrollable container — body.scrollHeight ≈ innerHeight.
      // Capturing the full body would make texture coordinates exceed [0,1] when the
      // panel is in the lower half of the screen, showing the wrong (clamped) background.
    ...
    _startLoop() {
      ...
      const draw = () => {
        if (this._dead || !this._prog) return;
        this._draw();
    ...
    handle.addEventListener('mousedown', e => {
      ...
      const onUp = () => {
        ...
        if (moved) {
          ...
          if (panel._glass) panel._glass._schedCapture(200);
        }
