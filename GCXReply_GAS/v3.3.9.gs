...
(function () {
  ...
  async function relayAbmReply_(ticketId, htmlBody) {
    ...
    logAbmRelayResult_(ticketId, match.caseId, domain, 'pending', 0, '', plainText);
    let resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    ...
    logAbmRelayResult_(ticketId, match.caseId, domain, ok ? 'success' : 'failed', attempts, ok ? '' : 'exhausted retries', plainText);
  ...
  function logAbmRelayResult_(ticketId, caseId, marketplace, status, attempts, lastError, messageText) {
    GM_xmlhttpRequest({
      method: 'POST',
      url: GAS_URL,
      headers: { 'Content-Type': 'application/json' },
      data: JSON.stringify({ action: 'logAbmRelay', ticketId, caseId, marketplace, status, attempts, lastError, messageText }),
      timeout: 15000,
  ...
  // the durable GAS-backed log rather than in-memory state. Clicking it
  // triggers an immediate retry sweep instead of waiting for the periodic one.
  ...
  async function updateAbmBadge_() {
    ...
    if (!_abmBadgeEl_) {
      ...
      _abmBadgeEl_.style.cssText = `position:fixed;bottom:16px;right:16px;z-index:2147483647;
        background:#c0392b;color:#fff;padding:8px 14px;border-radius:20px;
        font:600 12px -apple-system,sans-serif;cursor:pointer;box-shadow:0 4px 16px rgba(0,0,0,.3);`;
      _abmBadgeEl_.title = 'Click to retry now';
      _abmBadgeEl_.onclick = () => sweepAbmRelayFailures_();
      document.body.appendChild(_abmBadgeEl_);
    }
    _abmBadgeEl_.textContent = `⚠ ${pending.length} ABM ${pending.length === 1 ? 'reply' : 'replies'} not yet delivered — click to retry`;
  }
  ...
  async function sweepAbmRelayFailures_() {
    ...
    for (const item of pending) {
      if (!item.caseId || !item.marketplace || !item.messageText) continue;
      const base = `https://sellercentral.${item.marketplace}`;
      const fresh = await resolveScCaseDirect_(base, item.caseId).catch(() => null);
      if (!fresh) continue; // this browser has no usable SC session for that marketplace/case right now
      const resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
      if (!resourceVersion) continue;
      const ok = await sendScMessage_(base, item.caseId, fresh.marketplaceId, null, '', fresh.csrfToken, resourceVersion, item.messageText, []);
      if (ok) {
        recovered++;
        logAbmRelayResult_(item.ticketId, item.caseId, item.marketplace, 'success', (Number(item.attempts) || 1) + 1, '', item.messageText);
      }
    }
    updateAbmBadge_();
