...
(function () {
  ...
  // unconfirmed field.
  function sendScMessage_(base, caseId, marketplaceId, orderId, subject, csrfToken, resourceVersion, text) {
    return new Promise(resolve => {
      GM_xmlhttpRequest({
        method: 'POST',
  ...
  async function relayAbmReply_(ticketId, htmlBody) {
    ...
    let resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
    let ok = resourceVersion && await sendScMessage_(base, match.caseId, match.marketplaceId, orderId, ticket.subject, match.csrfToken, resourceVersion, plainText);
    if (!ok) {
      _scResourceVersion = null;
      resourceVersion = await fetchScResourceVersion_(base).catch(() => null);
      ok = resourceVersion && await sendScMessage_(base, match.caseId, match.marketplaceId, orderId, ticket.subject, match.csrfToken, resourceVersion, plainText);
    }
    logStep_(ok
