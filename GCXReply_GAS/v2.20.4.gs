        ...
        try {
          ...
          fetchScBuyerStats_(orderId, data.order?.SalesChannel, data.address?.CountryCode, stats => {
            ...
            const updated = Object.assign({}, data, {
              // Keep SP-API purchase count if available; SC is fallback
              totalPurchases: data.totalPurchases ?? stats.totalPurchases,
              // Use SC refund count when available (SC sees actual refund events)
              totalRefunds:   stats.totalRefunds != null ? stats.totalRefunds : (data.totalRefunds ?? 0),
